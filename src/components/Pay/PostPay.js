...
function PostPay() {
  ...
  // 배송 정보 입력
  const handleForm = (e) => {
    if (e.target.id === "pay_name") {
      // 수령인 이름
      setPayForm((prevPayForm) => ({
        ...prevPayForm,
        name: e.target.value,
      }));
    } else if (e.target.id === "pay_phone") {
      // 수령인 연락처
      ...
      const phoneNum =
        onlyNum.length < 4
          ? onlyNum
          : onlyNum.length < 7
          ? onlyNum.substr(0, 3) + "-" + onlyNum.substr(3)
          : onlyNum.length < 11
          ? onlyNum.substr(0, 3) +
            "-" +
            onlyNum.substr(3, 3) +
    ...
    } else if (e.target.id === "pay_add") {
      // 배송지 주소
    } else if (e.target.id === "pay_add_detail") {
      // 배송지 상세주소
    ...
    <div className="PP-_wrap">
      <div className="PP-title">
        ...
        <span className="PP-code">
          {payInfo.category_code}-{payInfo.product_code}
        </span>
      </div>
            ...
            <div className="PP-form">
              <div className="PP-form_wrap">
                <input
                  ...
                  name="pay_add_detail"
                  maxLength="50"
                  value={payForm.detail_address}
                  onChange={handleForm}
                  placeholder="상세주소"
                  disabled={payForm.address === ""}
                  required
                />
              </div>
            </div>
      ...
      <div className="PP-pay_user_wrap">
        ...
        <div className="PP-pay_user">
          <span className="PP-pay_coupon_wrap">
            ...
            <span className="PP-pay_user_use">
              {pay.coupon_price.toLocaleString("ko-KR")}&nbsp;원
            </span>
            <button className="PP-pay_coupon_button" onClick={couponChoice}>
              쿠폰 선택
            </button>
          </span>
          <span className="PP-pay_mile_wrap">
            <label className="PP-pay_user_title" htmlFor="pay_mile">
              마일리지 사용
            </label>
            <span className="PP-pay_mile">
              <input
                className="PP-pay_user_use"
                type="number"
                id="pay_mile"
                name="pay_mile"
                value={pay.mile === 0 ? "" : pay.mile}
                onChange={handleForm}
                placeholder="0"
                required
              />
            ...
            <span className="PP-pay_user_mile">
              {userInfo.mile.toLocaleString("ko-KR")}원 사용 가능
            </span>
          </span>
        </div>
      </div>
